...
typed formal calculi is {\em logical relations}.  Logical relations are % TODO through?
...
a term is ``good'' if it behaves ``well'' in all ``good'' contexts.
...
Their relation $\E$ also is defined by a recursive equation,
...
because of the mention that during handling the handler in the resumption is wrapped
around a subterm of the original handled computation.
...
It is feasible that such an argument could also work in our semantics with
a carefully selected term size function.
...
It seems that a tool for proof search is used and the proof is not spelled out in detail,

...
As standard, we work modulo $α$-equivalence and distinguish separate instances
of metavariables by using subscripts, superscripts, or adding primes.

The evaluation contexts encode a left-to-right evaluation order.
In \cref{freeness} we define a property of evaluation contexts
called {\em $n$-freeness}, which means that an operation performed inside this
context would be handled by the $n$-th handler outside of the context.
This property is used in the reduction semantics (\cref{reduction}) for the calculus.

In \cref{types} the type system is introduced.
We have three kinds: types, effects, and rows.
We use $α$ to range over type variables.
Contexts $Γ$ are mappings from expression variables to types
and type contexts $Δ$ are mappings from type variables to kinds.
We additionally have the concept of a type-level substitution
$Δ ⊢ δ ∷ Δ'$, which maps type variables in $Δ'$ to types of the appropriate kinds,
well-formed in $Δ$.

...
This requires use of monadic style, which among other things causes
stratification of code into two styles and does not always cooperate well with
the language's native facilities.
Moreover, monads do not compose in general, so modular programming with monads introduces some overheads.

In recent years a new promising approach has emerged: \textit{algebraic effects and handlers} \cite{Plotkin_2013}.
Algebraic effects allow programmers to express common computational effects as library code
and use them in the usual direct style.
This is done by decoupling effects into purely syntactic {\em operations} with their signatures
...
This work proves termination of evaluation (also known as normalization) of a
particular language equipped with algebraic effects.
...
of nontermination.
For example, the Koka programming language\cite{koka} implements this feature.
...
\caption{
	...
	but these can be defined using Church encodings.
	We set $\textit{tick} = λx.\,\Lift{\Do ()}$. The semicolon is standard syntax sugar:
	$e_1;e_2$ is expanded out to $(λx.\,e_2)\;e_1$ with $x$ not free in $e_2$.
...
More graphically,
we can associate with an element in $\E⟦τ/ρ⟧_η$ what we will call its {\em reduction trace}
% TODO: quotes?
– a tree where nodes are expressions in $\E⟦τ/ρ⟧_η$,
leaves are values in $⟦τ⟧_η$, and edges are either multi-step reductions
...
which contains the handler for $r$,

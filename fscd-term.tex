...
and use them in the usual direct style (as opposed to monadic style, % TODO CITE comprehending monads
...
This access to the continuation, while powerful, might also be a recipe for disaster. % TODO non sequitur ?
...
the effect system, provided that we have sufficiently identified possible sources % TODO Koka
of nontermination.
...
However, it is hard to give logical interpretations to effect annotations % TODO: cite? A Substructural Type Systemfor Delimited Continuations?
...
typed formal calculi are {\em logical relations}.  Logical relations are % TODO through?
...
In this work we will define logical predicates\footnote{
	In the context of normalization, logical predicates have many names, including:
	(Tait's) reducibility predicates, reducibility candidates, and saturated sets.
}
to prove termination
of evaluation in our language.
%An interesting point % TODO interesting point? ;/
A novelty of our approach
is that the definition of the logical predicates
is recursive, in other words it is a solution to a fixed-point equation.
To our knowledge, this is the first use of this technique in the
area of algebraic effects
...
Note that the type system enables {\em row polymorphism}, % TODO: CITE
...
the fixed-point equation $\E⟦τ/ρ⟧_η(X) = X$ has a least solution.\footnote{
	...
	The proofs of compatibility lemmas would fail:
	they graft different reduction traces to the leaves of a
...
\section{Applicability to other settings}

The calculus considered so far is fairly simple.
In particular, we cannot have multiple operations grouped into one effect
and we do not labels (analogous to prompts for delimited continuations),
which allow us to have completely independent sets of effect-related constructs in our programs.

\section{Related work}

We have already discussed \cite{hwc}, where the interpretations of effects
and the idea of a relation on control-stuck terms comes from.
One thing that was not mentioned is that their relation is {\em biorthogonal}
– there is a relation on full program evaluation contexts.
The relation on expressions is defined extensionally:
a term is good if it behaves well in a good context.
Noticing that this is unnecessary leads to simpler definitions and proofs.
More importantly, the intensional direct-style definition of good terms leads to
better intuition, in particular allows us to consider reduction traces.
However, opting for direct-style logical relations would possibly not simplify the definitions
...
They also have a recursive definition and have to take a fixed-point solution,
however they interpret it {\em coinductively},
i.e. as the greatest fixed point.

In our setting, a coinductive definition would lead to
expressions in $\E$ having reduction traces with potentially infinite depth.
Note that if a reduction trace has no leaves, so the reduction never reaches a value,
...
an expression in $\E⟦τ/τ_1 \Rightarrow τ_2⟧_η$ and produce something in $\E⟦τ/ι⟧_η$, which cannot get
stuck and simply evaluates to a value of type $τ$.
This seems impossible if
the former can never evaluate to a value and the $τ$ can even be arbitrary.
